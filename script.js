...
// --- BILDER (Angepasst an .webp) ---
const characterImg = new Image();
characterImg.src = "assets/3DPingsta.webp"; // [1] Geändert zu .webp

const chainsawImg = new Image();
chainsawImg.src = "assets/chainsaw.webp";   // [1] Geändert zu .webp

const ak47Img = new Image();
ak47Img.src = "assets/AK47.webp";           // [1] Geändert zu .webp

const bgImg = new Image();
bgImg.src = "background.png"; 
// Hinweis: Da diese Datei im Screenshot fehlt, wird das Spiel automatisch
// den eingebauten Farbverlauf nutzen (siehe unten in background.draw).

// --- AUDIO (Angepasst an vorhandene Dateien) ---
const bgMusic = new Audio("LIEBLOS.mp3");   // [1] Geändert auf vorhandene Datei
bgMusic.loop = true;
bgMusic.volume = 0.6;

// Fatality Sound (Name aus Screenshot übernommen)
const fatalityAudio = new Audio("Fatality (Mortal Kombat) - QuickSounds.com.mp3"); // [1]
fatalityAudio.volume = 0.4;

// Waffensounds
const shotAudio = new Audio("shot.mp3"); // [1]
shotAudio.volume = 0.2;

const chainsawAudio = new Audio("chainsaw.mp3"); // [1]
chainsawAudio.volume = 0.2;
chainsawAudio.loop = true; // Kettensäge rattert durchgehend
...
function drawFatalitySequence() {
    ...
    if (gameOverTimer < 90) {
        ...
        if (gameOverTimer === 90) {
            ...
            // Chainsaw Sound auch stoppen falls an
            chainsawAudio.pause();
...
function drawLostWeapon() {
    ...
    if (img.complete && img.naturalHeight !== 0) {
        if (lostWeapon.type === "ak47") {
            ctx.scale(-1, 1); // AK beim Wegfliegen normal
        } else {
            ctx.scale(-1, -1); // Chainsaw war anders gedreht
        }
    ...
    draw: function() {
        ...
        if (hasChainsaw || hasAK47) {
            ...
            if (img.complete && img.naturalHeight !== 0) {
                ...
                if (hasAK47) {
                    // AK47 Ausrichtung korrigieren
                    ctx.scale(-1, 1);
                    ctx.drawImage(img, 5, -5 - bobbing, 60, 30);
                    ...
                    if (frames % 8 < 3) {
                        ...
                        ctx.globalAlpha = 0.8;
                        ctx.beginPath();
                        ctx.arc(65, 10 - bobbing, 8 + Math.random()*5, 0, Math.PI*2);
                        ctx.fill();
                    }
                } else {
                    // Kettensäge Ausrichtung
                    ctx.scale(-1, -1);
...
const background = {
    ...
    draw: function() {
        // Fallback-Logik, falls background.png fehlt
...
function update() {
    ...
    if (currentState === STATE_PLAYING) {
        ...
        // AUDIO MANAGEMENT FÜR CHAINSAW
        if (hasChainsaw) {
            // Nur abspielen wenn noch nicht läuft
            if (chainsawAudio.paused) chainsawAudio.play().catch(e => {});
        } else {
            // Stoppen wenn man sie nicht hat
            if (!chainsawAudio.paused) chainsawAudio.pause();
        }
    } else {
        // Im Menu/GameOver Ruhe
        if (!chainsawAudio.paused) chainsawAudio.pause();
    }
...
function updateBullets() {
    ...
    if (hasAK47) {
        ...
        if (shootTimer > 8) { // Feuerrate
            ...
            let muzzleY = bird.y + bird.h/2 + 20; 
